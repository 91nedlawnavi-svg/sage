...
| **Felt continuity** · now | Relevant episodes, associations, and contradictions influence ordinary replies naturally. Past chats can be reopened, continued, renamed, archived, restored, deleted permanently with typed confirmation, and given a per-chat text model without splitting lifetime memory. Safe answer retries reuse the saved turn. | Feeling like one continuing intelligence across daily life while models remain replaceable engines. | Memory must improve the exchange, not announce retrieval; explicit model failure must stay honest; deletion must name its exact local boundary. |
